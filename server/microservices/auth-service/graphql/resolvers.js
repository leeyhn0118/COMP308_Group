      ...
      try {
        ...
        return {
          ...
          if (error.name === 'ValidationError') {
              throw new GraphQLError(`Registration validation failed: ${error.message}`, { extensions: { code: 'BAD_USER_INPUT' } });
          }
          throw new GraphQLError('Failed to register user', { extensions: { code: 'INTERNAL_SERVER_ERROR' } });
      }
    },
    logout: (_, __, { res }) => {
  ...
  User: {
    __resolveReference: async (reference) => {
      ...
      if (!mongoose.Types.ObjectId.isValid(reference.id)) {
          ...
          throw new GraphQLError('Invalid User ID format in reference', { extensions: { code: 'BAD_USER_INPUT' } });
      }
      try {
        ...
        return user;
          ...
          throw new GraphQLError('Could not resolve user reference', { extensions: { code: 'INTERNAL_SERVER_ERROR' } });
      }
    }
  }
};
